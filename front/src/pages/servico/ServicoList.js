...
/* ── style tokens ── */
const tableStyle = { width: "100%", borderCollapse: "separate", borderSpacing: 0, animation: "fadeIn 0.3s ease", minWidth: 600 };
    ...
    <div className="page-card">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Gerencie os serviços oferecidos pelo pet shop.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="table-responsive">
          <table style={tableStyle}>
            ...
            <tbody>
              {servicos.map((s) => (
                <tr key={s.id} style={{ transition: "background 0.15s ease" }} onMouseEnter={(e) => e.currentTarget.style.background = "#f7faf8"} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                  <td style={tdStyle}><span style={idPill}>#{s.id}</span></td>
                  <td style={{ ...tdStyle, whiteSpace: "normal", minWidth: 140 }}>{s.descricao}</td>
                  <td style={tdStyle}><span style={priceStyle}>{formatCurrency(s.preco)}</span></td>
                  <td style={tdStyle}><span style={durationBadge}>{formatDuration(s.duracaoHoras)}</span></td>
                  <td style={{ ...tdStyle, textAlign: "right" }}>
                    <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                      <Link to={`/servico/editar/${s.id}`} style={btnEdit}>
                        <i className="bi bi-pencil-fill" style={{ fontSize: 12 }}></i> Editar
                      </Link>
                      <button onClick={() => handleDeletar(s.id)} style={btnDel}>
